...
def sync_sheets():
    ...
    try:
        ...
        def c(col): return " ".join(df_sheet[col].dropna().astype(str).apply(lambda x: x.split('.')[0].strip()))
        st.session_state.search_codes = c('snipe_list')
        st.session_state.inventory_codes = c('inventory_list')
...
def perform_scan():
    ...
    if m_df is not None:
        ...
        elif score >= 40: cmd, clz, tip = "🤏 少量買進", "buy-signal", "⚠️ 處於震盪區。"
        ...
        with st.expander("大盤趨勢圖表"): st.plotly_chart(plot_advanced_chart(m_df, "TAIEX 指數"), use_container_width=True)
    ...
    for sid in all_codes:
        ...
        if st.session_state.notified_status.get(sid) != sig_lvl:
            ...
            if should_send:
                discord_msg = (f"{msg_header}\n**──────────────────**\n **原因**: `{reason}`\n **現價**: `{last['close']:.2f}` (5MA乖離: {last['bias_5']:.2f}%)\n"
                               f" **評分**: `{last['score']} 分` | **預估量比**: `{last['vol_ratio']:.2f}x`\n"
                               f" **提醒**: {last['warning']}\n**──────────────────**")
        ...
        with st.expander(f"查看 {sid} {name} 詳細分析"):
            st.plotly_chart(plot_advanced_chart(df, f"{sid} {name}"), use_container_width=True)
            
        scan_results.append({"類別": "庫存" if is_inv else "狙擊", "代碼": sid, "名稱": name, "分數": last["score"], "型態": last["pattern"], "提醒": last["warning"]})
