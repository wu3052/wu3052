...
def plot_advanced_chart(df, title=""):
    ...
    # 3. 關鍵位
    for key, color in [("upward_key", 'rgba(235,77,75,0.4)'), ("downward_key", 'rgba(46,204,113,0.4)')]:
        if key in df_plot.columns:
            fig.add_trace(go.Scatter(x=df_plot["date"], y=df_plot[key], name=key, line=dict(color=color, dash='dash')), row=1, col=1)
    
    # 4. 噴發標記 (🚀) --- 終極修正版 ---
    if "is_first_breakout" in df_plot.columns:
        # 強制轉換為布林值，解決 0/1 或 NaN 的判斷問題
        ...
        if not breakouts.empty:
            fig.add_trace(go.Scatter(
                x=breakouts["date"], 
                y=breakouts["low"] * 0.96, # 標記在最低價下方 4%
                mode="markers+text", 
    ...
    if "hist" in df_plot.columns:
        colors = ['#ff4b4b' if v >= 0 else '#28a745' for v in df_plot["hist"].fillna(0)]
