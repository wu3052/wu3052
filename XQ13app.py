...
if 'last_notify_time' not in st.session_state:
    st.session_state.last_notify_time = {}
...
# 🔹 數據獲取模組
# =====================
@st.cache_data(ttl=300) # 縮短快取時間以利盤中監控
...
def analyze_strategy(df):
    ...
    if row["close"] > row["ma144_60min"] and prev_row["close"] <= prev_row["ma144_60min"]: warnings.append("🚀 站上60分144MA")
    elif row["close"] < row["ma144_60min"] and prev_row["close"] >= prev_row["ma144_60min"]: warnings.append("🩸 跌破60分144MA")
    
    if row["close"] > row["ma5"] and prev_row["close"] <= prev_row["ma5"]: warnings.append("🏹 站上5MA")
    elif row["close"] < row["ma5"] and prev_row["close"] >= prev_row["ma5"]: warnings.append("⚠️ 跌破5MA")
    
    if row["close"] > row["ma10"] and prev_row["close"] <= prev_row["ma10"]: warnings.append("🔍 站上10MA")
    elif row["close"] < row["ma10"] and prev_row["close"] >= prev_row["ma10"]: warnings.append("🚨 跌破10MA")

    if row["vol_ratio"] > 2.0: warnings.append("🔥 量能爆發")
    if row["star_signal"]: warnings.append("⭐ 星級發動點")
...
with st.sidebar:
    ...
    st.divider()
    auto_monitor = st.toggle("🛰️ 啟動全天候自動監控", value=False)
    interval = st.slider("掃描頻率 (分鐘)", 1, 30, 5)
...
def run_scanner():
    ...
    # 處理代碼清單
    target_list = [c for c in re.split(r'[\s\n,]+', st.session_state.search_codes) if c]
    inv_list = [c for c in re.split(r'[\s\n,]+', st.session_state.inventory_codes) if c]
    all_targets = list(set(target_list + inv_list))
    
    stock_info = get_stock_info()
    
    st.subheader(f"📊 盤中監控報告 ({datetime.now().strftime('%H:%M:%S')})")
    ...
    for sid in all_targets:
        ...
        # 判斷標籤
        tag = ""
        if sid in inv_list and sid in target_list: tag = "【庫存/狙擊】"
        elif sid in inv_list: tag = "【庫存通知】"
        elif sid in target_list: tag = "【狙擊個股通知】"

        # 輸出顯示
        st.write(f"{tag} **{sid} {s_name}**: 現價 {last['close']:.2f} | {last['warning']}")
        
        # LINE 通知觸發 (若有顯著警告且一小時內未重複發送相同代碼)
        if line_token and last['warning'] != "趨勢穩定中":
            msg = f"\n{tag}\n代號: {sid}({s_name})\n現價: {last['close']:.2f}\n戰情: {last['warning']}\n位置: {last['position_type']}"
    ...
    if scan_results:
        st.divider()
        results_df = pd.DataFrame(scan_results).sort_values(by="分數", ascending=False)
        st.dataframe(results_df, use_container_width=True)

# 執行區域
if auto_monitor:
    st.toast("🛰️ 自動監控模式已開啟...")
    while True:
        run_scanner()
    ...
    st.info("💡 請於左側輸入代碼並點擊『執行掃描』或開啟『自動監控』。")

st.sidebar.markdown("---")
st.sidebar.caption("股票狙擊手 2026/04/14 雲端自動化版")