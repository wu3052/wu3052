...
# --- 4. AI 模組與配額優化 ---
@st.cache_data(ttl=3600) # 緩存 AI 分析結果 1 小時，避免重複呼叫觸發 429
def analyze_sentiment_and_news(sid, name):
    ...
    try:
        ...
        prompt = (f"分析台股 {sid} {name} 的市場情緒。 "
                  f"重點關鍵字：{keywords}。請極簡短給出：1.情緒分數(0-100) "
                  f"2.一句話摘要熱度，判斷是否過熱。")
        
        response = model.generate_content(prompt)
        time.sleep(2) # 強制延遲 2 秒，防止免費版連發觸發頻率限制
        return response.text, "AI 分析完成"
    except Exception as e:
        if "429" in str(e):
            return "⚠️ AI 頻率受限 (API 429)，改由技術指標為主。請稍後手動掃描。", "RateLimit"
        return f"AI 暫時無法連接: {str(e)}", "Error"
...
def analyze_strategy(df, is_market=False):
    ...
    if not pd.isna(row["upward_key"]) and row["close"] > row["upward_key"] and prev["close"] <= row["upward_key"]: buy_pts.append("破關鍵位")
    ...
    if not is_market:
        score = score * (st.session_state.market_score / 100)
...
def plot_advanced_chart(df, title=""):
    ...
    for ma, color in {5: '#2980b9', 10: '#f1c40f', 20: '#e67e22'}.items():
        fig.add_trace(go.Scatter(x=df_plot["date"], y=df_plot[f"ma{ma}"], name=f"{ma}MA", line=dict(color=color)), row=1, col=1)
    fig.add_trace(go.Bar(x=df_plot["date"], y=df_plot["hist"], name="MACD"), row=2, col=1)
...
def perform_scan():
    ...
    if m_df is not None:
        ...
        clz = "buy-signal" if score >= 40 else "sell-signal"
        st.markdown(f"<div class='status-card {clz}'>大盤狀態: {'穩定' if score >= 40 else '惡劣'} (環境係數: {score:.1f})</div>", unsafe_allow_html=True)
    ...
    for sid in all_codes:
        ...
        border = "#ff4b4b" if last["sig_type"] == "BUY" else "#28a745"
        
        st.markdown(f"""
        <div class="dashboard-box" style="border-left: 10px solid {border}; margin-bottom:10px; text-align:left;">
            <b>{'🎯 狙擊' if is_snipe else '📦 庫存'} {sid} {name} | 現價: {last['close']:.2f}</b> | 評分: {last['score']:.1f}<br>
            <small>建議倉位: {suggest_pos} | 提醒: {last['warning']} | 預估量比: {last['vol_ratio']:.2f}x</small><br>
            <div style="background:#f1f5f9; padding:5px; border-radius:5px; font-size:0.85em; margin-top:5px;">🤖 {ai_summary}</div>
        </div>
        """, unsafe_allow_html=True)
        
        with st.expander(f"查看 {sid} 分析圖"):
            st.plotly_chart(plot_advanced_chart(df, f"{sid} {name}"), use_container_width=True)
        
        processed_stocks.append({"sid": sid, "name": name, "score": last["score"]})
