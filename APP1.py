...
if 'stop_discord' not in st.session_state: st.session_state.stop_discord = False # 初始化停止按鈕狀態
...
def send_discord_message(msg):
    """整合需求 1: 加入手動停止傳送訊息按鈕判斷"""
    # 如果使用者手動開啟了停止傳送，則直接返回，不發送訊息
...
def analyze_strategy(df, is_market=False):
    ...
    is_first_breakout = (
        was_tangling and 
        row["close"] > max([prev["ma5"], prev["ma10"], prev["ma20"]]) and 
    ...
    if sell_pts:
        ...
        if is_no_new_high:
            warning_list.append("頭部位階跌破(1日不創新高)")
        warning_list.extend(sell_pts)
    # 如果有買入訊號且無賣出訊號，判斷「底部位階支撐」
    elif buy_pts:
        is_no_new_low = row["low"] >= prev["low"]
        if is_no_new_low:
            warning_list.append("底部位階支撐(1日不創新低)")
        warning_list.extend(buy_pts)
    else:
        warning_list.append("趨勢穩定中")
    ...
    # 考慮位階加減分
    if "底部位階支撐(1日不創新低)" in warning_list: score += 5
    if "頭部位階跌破(1日不創新高)" in warning_list: score -= 5
    ...
    df.at[last_idx, "warning"] = " | ".join(warning_list)
    
    sig = "HOLD"
    if buy_pts and (not sell_pts or score > 55): sig = "BUY"
    elif sell_pts and (not buy_pts or score < 45): sig = "SELL"
...
with st.sidebar:
    ...
    st.divider()
    # 整合需求 1: 手動停止傳送訊息按鈕 (亮色風格 toggle)
    st.session_state.stop_discord = st.toggle("🚫 停止 Discord 訊息傳送", value=st.session_state.stop_discord, help="勾選後，系統將不再發送任何訊息到 Discord，直到取消勾選。")
    st.divider()

    interval = st.slider("監控間隔 (分鐘)", 1, 30, 5)
    
    # 整合需求 2: 將開啟盤中自動監控與時間判斷整合
    auto_monitor = st.checkbox("🔄 開啟盤中自動監控 (依開盤時間自動執行)", value=True)
    
    analyze_btn = st.button("🚀 執行即時掃描", use_container_width=True)
    
    # 狀態顯示
    market_status = is_market_open()
    status_text = "🔴 盤中交易" if market_status else "🟢 市場收盤"
        ...
        for future in future_to_sid:
            ...
            try:
                ...
                processed_stocks.append({
                    ...
                    "is_inv": is_inv, "is_snipe": is_snipe, "score": last["score"], "warning": last["warning"], "pattern": last["pattern"], "pattern_desc": "在糾結均線後，股價選擇帶量突破..." if last["is_first_breakout"] else "趨勢穩定中..."
                })
            except Exception as e:
...
# 如果勾選了「自動化監控」
    ...
    if is_market_open():
        ...
        # 非開盤時間：自動停止掃描與傳訊，顯示月亮圖示
        with placeholder.container():
            st.warning(f"🌙 目前非台灣市場交易時段 (09:00~13:35)，自動監控已休眠。\n目前時間: {get_taiwan_time().strftime('%H:%M:%S')}")
        # 在非交易時段，降低重整頻率 (例如每 5 分鐘檢查一次是否開盤即可)
        time.sleep(300) 
        st.rerun()
else:
    # 沒勾自動也沒按掃描，僅靜態顯示
    with placeholder.container():
        st.info("若要啟動監控，請按手動掃描或勾選開啟自動監控。")
