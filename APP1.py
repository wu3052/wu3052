...
if analyze_btn:
    ...
    if is_market_open():
        ...
        # 收盤期間顯示最後一次掃描結果，不進入 sleep 循環，也不再發送 Discord 訊息
        with placeholder.container(): perform_scan()
        st.warning("🌙 目前非開盤時間 (09:00~13:35)，自動監控已進入休眠模式。")
else:
    # 手動模式
    with placeholder.container(): perform_scan()
