...
def analyze_strategy(df, is_market=False):
    ...
    if not pd.isna(row["upward_key"]) and row["close"] > row["upward_key"] and prev["close"] <= row["upward_key"]: buy_pts.append("站上死叉關鍵位")
    ...
    if not pd.isna(row["downward_key"]) and row["close"] < row["downward_key"] and prev["close"] >= row["downward_key"]: sell_pts.append("跌破金叉關鍵位")
    ...
    elif risk_volatility > 3.0: pos_advice = "建議配置: 3~5% (高波動)"
...
def plot_advanced_chart(df, title=""):
    ...
    # 修正顏色邏輯：確保使用 df_plot 內的數據
    macd_colors = ['#ff4b4b' if val >= 0 else '#28a745' for val in df_plot["hist"]]
    fig.add_trace(go.Bar(x=df_plot["date"], y=df_plot["hist"], name="MACD", marker_color=macd_colors), row=2, col=1)
    
    fig.update_layout(height=650, title=title, template="plotly_white", xaxis_rangeslider_visible=False)
    return fig
...
with st.sidebar:
    ...
    auto_monitor = st.checkbox("🔄 開啟自動監控", value=True)
...
def perform_scan():
    ...
    if m_df is not None:
        ...
        score = m_last["score"]
        clz = "buy-signal" if score >= 60 else ("sell-signal" if score <= 40 else "neutral-signal")
        st.markdown(f"<div class='status-card {clz}'>大盤評分: {score}</div>", unsafe_allow_html=True)
        with st.expander("📊 查看加權指數圖表"):
            st.plotly_chart(plot_advanced_chart(m_df, "TAIEX"), use_container_width=True)
    ...
    for sid in all_codes:
        ...
        # 處理 Discord 通知
        sig_lvl = f"{last['sig_type']}_{'BOOM' if last['vol_ratio'] > 1.8 else 'NOR'}"
        if st.session_state.notified_status.get(sid) != sig_lvl:
            discord_msg = f"🏹 【{name} {sid}】\n價格: {last['close']}\n評分: {last['score']}\n訊號: {last['warning']}"
    ...
    # 渲染結果
    for group_name, is_key in [("🎯 狙擊清單", "is_snipe"), ("📦 庫存清單", "is_inv")]:
        st.subheader(group_name)
        targets = [s for s in processed_stocks if s[is_key]]
        for item in targets:
            l, sid, n = item["last"], item["sid"], item["name"]
            st.markdown(f"""<div class="dashboard-box" style="border-left:10px solid #ff4b4b; text-align:left; margin-bottom:10px;">
                <b>{sid} {n} | 現價: {l['close']:.2f} | 評分: {l['score']}</b><br>
                提醒: {l['warning']} | {l['pattern']}
...
if market_active and auto_monitor:
    st.sidebar.success("● 交易時段：系統掃描中")
    perform_scan()
    next_run = (get_taiwan_time() + timedelta(minutes=interval)).strftime('%H:%M:%S')
    st.sidebar.write(f"⏳ 下次更新：**{next_run}**")
    time.sleep(interval * 60)
    st.rerun()

else:
    if market_active: st.sidebar.warning("○ 交易時段：自動監控關閉")
    else: st.sidebar.info("○ 非交易時段：監控暫停")
    
    if analyze_btn:
        perform_scan()
        st.toast("手動掃描完成")
    elif st.session_state.event_log:
        # 確保畫面不空白
        perform_scan()
    else:
        st.info("💡 請點擊「🚀 執行即時掃描」或等待開盤。")

    if auto_monitor and not market_active:
        time.sleep(3600)
        st.rerun()
